...
public class HHRequestProcessor extends RequestProcessor {
    ...
    public HttpResponse getSession(String login, String password) throws Exception {
        return this.processPostRequest(BASE_URL + PHOENIX_AJAX,
                this.bodyBuilder.generateBody(new PlayerInfos(login, password)));
    }
    ...
    public String getHaremContent(HHSession session) throws Exception {
        return this.readAllPageContent(BASE_URL + HAREM, session.getCookieHeader());
    }
    ...
    public OpponentInfoResponse getOpponentInfo(HHSession session, String opponentId) throws Exception {
        return this.processPostRequest(OpponentInfoResponse.class, BASE_URL + AJAX,
                this.bodyBuilder.generateBody(new ActionLeaguesGetOpponentInfo(opponentId)), session.getCookieHeader()).getEntity();
    }

    public SalaryResponse getSalary(HHSession session, int which) throws Exception {
        return this.processPostRequest(SalaryResponse.class, BASE_URL + AJAX,
                this.bodyBuilder.generateBody(new ActionGirlSingleSalary(which)), session.getCookieHeader()).getEntity();
    }

    public SalaryResponse getAllSalaries(HHSession session) throws Exception {
        return this.processPostRequest(SalaryResponse.class, BASE_URL + AJAX,
                this.bodyBuilder.generateBody(new ActionGirlAllSalaries()), session.getCookieHeader()).getEntity();
    }

    public DraftResponse draftChampionFight(HHSession session, Integer idChampion, List<Integer> girlsToKeep) throws Exception {
        return this.processPostRequest(DraftResponse.class, BASE_URL + AJAX,
                this.bodyBuilder.generateBody(new ActionChampionsTeamDraft(idChampion, girlsToKeep)), session.getCookieHeader()).getEntity();
    }
    ...
    public TeamBattleResponse fightChampion(HHSession session, Currency currency, Integer championId, List<Integer> girls) throws Exception {
        return this.processPostRequest(TeamBattleResponse.class, BASE_URL + AJAX,
                this.bodyBuilder.generateBody(new ActionTeamBattleChampion(currency, championId, girls)), session.getCookieHeader()).getEntity();
    }

    public Response startMission(HHSession session, Mission mission) throws Exception {
        return this.processPostRequest(SalaryResponse.class, BASE_URL + AJAX,
                this.bodyBuilder.generateBody(new ActionMissionStartMission(mission.getIdMission(), mission.getIdMemberMission())), session.getCookieHeader()).getEntity();
    }

    public Response getFinalMissionGift(HHSession session) throws Exception {
        return this.processPostRequest(SalaryResponse.class, BASE_URL + AJAX,
                this.bodyBuilder.generateBody(new ActionMissionGiveGift()), session.getCookieHeader()).getEntity();
    }

    public Response claimReward(HHSession session, Mission mission) throws Exception {
        return this.processPostRequest(SalaryResponse.class, BASE_URL + AJAX,
                this.bodyBuilder.generateBody(new ActionMissionClaimReward(mission.getIdMission(), mission.getIdMemberMission())), session.getCookieHeader()).getEntity();
    }

    public Response fightOpponentPlayer(HHSession session, BattlePlayer battlePlayer) throws Exception {
        return this.processPostRequest(Response.class, BASE_URL + AJAX,
                this.bodyBuilder.generateBody(new ActionBattlePlayer(battlePlayer)), session.getCookieHeader()).getEntity();
    }

    public Response fightOpponentMob(HHSession session, BattleMob battleMob) throws Exception {
        return this.processPostRequest(Response.class, BASE_URL + AJAX,
                this.bodyBuilder.generateBody(new ActionBattleMob(battleMob)), session.getCookieHeader()).getEntity();
    }

    public Response continueQuest(HHSession session, Long questId) throws Exception {
        return this.processPostRequest(Response.class, BASE_URL + AJAX,
                this.bodyBuilder.generateBody(new ActionQuestNext(questId)), session.getCookieHeader()).getEntity();
    }

    public Response upgradeStat(HHSession session, int statToUpgrade) throws Exception {
        return this.processPostRequest(Response.class, BASE_URL + AJAX,
                this.bodyBuilder.generateBody(new ActionUpgradeStat(statToUpgrade)), session.getCookieHeader()).getEntity();
    }

    public Response buyItem(Item item, HHSession session) throws Exception {
        return this.processPostRequest(Response.class, BASE_URL + AJAX,
                this.bodyBuilder.generateBody(new ActionBuyItem(item)), session.getCookieHeader()).getEntity();
    }

    public Response collectCompetitionRewards(Integer idContest, HHSession session) throws Exception {
        return this.processPostRequest(Response.class, BASE_URL + AJAX,
                this.bodyBuilder.generateBody(new ActionContestGiveReward(idContest)), session.getCookieHeader()).getEntity();
    }
